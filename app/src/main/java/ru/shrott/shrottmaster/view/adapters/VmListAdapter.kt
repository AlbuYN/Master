...
class VmListAdapter : RecyclerView.Adapter<VmViewHolder>(), IBaseListAdapter<IBaseItemVm> {
    ...
    override fun getItemViewType(position: Int) = mItems[position].getLayoutId()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VmViewHolder {

        val inflater = LayoutInflater.from(parent.context)
        val viewDataBinding = DataBindingUtil
            .inflate<ViewDataBinding>(inflater!!, viewType, parent, false)
    ...
    override fun replaceItems(newItems: List<IBaseItemVm>) {
        mItems = ArrayList(newItems)
